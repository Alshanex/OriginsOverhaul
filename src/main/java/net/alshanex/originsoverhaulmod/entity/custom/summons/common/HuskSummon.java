...
public class HuskSummon extends Husk implements MagicSummon, GeoAnimatable {
    private static final EntityDataAccessor<Boolean> DATA_IS_ANIMATING_RISE = SynchedEntityData.defineId(HuskSummon.class, EntityDataSerializers.BOOLEAN);

    public HuskSummon(EntityType<? extends Husk> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
        xpReward = 0;
    }

    public HuskSummon(Level level, LivingEntity owner, boolean playRiseAnimation) {
        this(ModEntities.SUMMONED_HUSK.get(), level);
        setSummoner(owner);
