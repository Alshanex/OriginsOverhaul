...
public class BalanzaBuena extends LivingEntity implements GeoEntity, AntiMagicSusceptible {
    ...
    public BalanzaBuena(EntityType<? extends BalanzaBuena> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);

    }

    public BalanzaBuena(Level level, LivingEntity owner, float damage, LivingEntity target, int spellPower, SchoolType school) {
        this(ModEntities.BALANZA_BUENA.get(), level);
        setOwner(owner);
        setDamage(damage);
        setTarget(target);
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            ...
            if (age == 110){
                ...
                if (target != null) {
                    float healAmount = this.spellPower;
                    MinecraftForge.EVENT_BUS.post(new SpellHealEvent(getOwner(), target, healAmount, this.school));
                    target.heal(healAmount);
                    target.addEffect(new MobEffectInstance(MobEffectRegistry.FORTIFY.get(), 20 * 120, this.spellPower - 1, false, false, true));
                    target.addEffect(new MobEffectInstance(MobEffectRegistry.HASTENED.get(), this.spellPower*20, 1));
                    Messages.sendToPlayersTrackingEntity(new ClientboundHealParticles(target.position()), target, true);
                }
            }
        }
        age++;
    ...
    private PlayState animationPredicate(software.bernie.geckolib.core.animation.AnimationState event) {
        ...
        if (age > 200) {
            ...
            controller.setAnimation(ANIMATION_GOOD);
        }
    ...
    private final RawAnimation ANIMATION_GOOD = RawAnimation.begin().thenPlay("animation.balanza.good");
