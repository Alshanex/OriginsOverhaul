...
public class WaterCutProjectile extends Projectile implements AntiMagicSusceptible {
    private static final EntityDataAccessor<Float> DATA_RADIUS = SynchedEntityData.defineId(WaterCutProjectile.class, EntityDataSerializers.FLOAT);
    ...
    public WaterCutProjectile(EntityType<? extends WaterCutProjectile> entityType, Level level) {
    ...
    public WaterCutProjectile(EntityType<? extends WaterCutProjectile> entityType, Level levelIn, LivingEntity shooter) {
    ...
    public WaterCutProjectile(Level levelIn, LivingEntity shooter) {
        this(ModEntities.WATER_CUT_PROJECTILE.get(), levelIn, shooter);
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            ...
            for (Entity entity : level().getEntities(this, this.getBoundingBox()).stream().filter(target -> canHitEntity(target) && !victims.contains(target)).collect(Collectors.toSet())) {
                ...
                MagicManager.spawnParticles(level(), ParticleHelper.SNOWFLAKE, entity.getX(), entity.getY(), entity.getZ(), 50, 0, 0, 0, .5, true);
    ...
    private void damageEntity(Entity entity) {
        if (!victims.contains(entity)) {
            DamageSources.applyDamage(entity, damage, ExampleSpellRegistry.WATER_CUT.get().getDamageSource(this, getOwner()));
            victims.add(entity);
        }
    }
    ...
    public void spawnParticles() {
        if (level().isClientSide) {
            ...
            for (int i = 0; i < width / step; i++) {
                ...
                level().addParticle(ParticleHelper.SNOWFLAKE, false, x + rotX + dx, y + dy, z + rotZ + dz, dx, dy, dz);
            }
        }
    }
    @Override
